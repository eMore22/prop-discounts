...
import { Plus, Edit, Trash2, Save, X, Calendar, Tag, Percent, Link as LinkIcon, Lock, LogOut } from 'lucide-react';
import PasswordChangeForm from '@/components/admin/PasswordChangeForm'; // Assuming this is correct
...
export default function AdminDashboard() {
  ...
  // NEW STATE TO STORE AUTHENTICATED ADMIN EMAIL
  ...
  // Fetch deals when authenticated
  useEffect(() => {
    if (isAuthenticated) {
      fetchDeals();
    }
  ...
  const checkAuth = async () => {
    try {
      // NOTE: Assuming '/api/admin/check' now returns the email of the logged-in user
      const response = await fetch('/api/admin/check');
      if (response.ok) {
        const data = await response.json();
        // Check if the API returns the email on success
        if (data.email) {
            setAdminEmail(data.email); 
        } else {
            // Fallback or ensure your check endpoint returns { email: '...' }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ON SUCCESSFUL LOGIN, STORE THE EMAIL DYNAMICALLY
      setAdminEmail(email); 
      setIsAuthenticated(true);
      setEmail('');
      setPassword('');
  ...
  // Login Screen
  if (!isAuthenticated) {
    // ... (Login screen code is unchanged) ...
    return (
      ...
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-3 gap-6 mb-8">
            ...
            {/* Left Column: Stats and Export Button */}
            ...
            <div className="lg:col-span-1">
              {/* PASSING THE DYNAMIC EMAIL AS A PROP */}
          ...
          {isEditing && (
             // ... (Editing form code is unchanged) ...
