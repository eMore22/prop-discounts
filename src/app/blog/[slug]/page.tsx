...
export default function BlogPostPage() {
  ...
  // params.slug can be string | string[] | undefined depending on Next version
  // We normalise it here without lowercasing — slugs in lib/blog.ts are already lowercase
  let rawSlug: string = '';
  ...
  // Decode URI encoding just in case (e.g. %2F) and lowercase to be safe
  const slug = decodeURIComponent(rawSlug).toLowerCase();

  // ── Post lookup ──────────────────────────────────────────────────────────
  // Compare both lowercase to guard against any capitalisation drift
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] py-20 text-center px-4">
        ...
        <p className="text-gray-500 text-sm mb-1">
          Could not find a post with slug: <code className="bg-gray-100 px-1 rounded">{slug || '(empty)'}</code>
        </p>
        <p className="text-gray-400 text-xs mb-6">
          Available slugs: {blogPosts.map(p => p.slug).join(', ')}
        </p>
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="bg-gradient-to-br from-[#060d1f] to-[#0d1f3c] text-white py-12">
        <div className="max-w-4xl mx-auto px-4">
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 text-slate-400 hover:text-white text-sm font-semibold mb-6"
          >
            <ArrowLeft className="w-4 h-4" /> Back to Blog
          </Link>

          <div className="flex flex-wrap items-center gap-3 mb-5">
            <span className="text-xs font-black px-3 py-1.5 rounded-full bg-blue-100 text-blue-700">
              {post.category}
            </span>
            {post.tags.slice(0, 2).map(tag => (
              <span
                key={tag}
                className="text-xs font-bold text-slate-400 bg-white/5 px-3 py-1.5 rounded-full"
              >
                {tag}
              </span>
            ))}
            {post.author && (
              <span className="text-xs font-bold text-blue-300 bg-blue-500/10 px-3 py-1.5 rounded-full">
                {post.author}
              </span>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-5 text-sm text-slate-400">
            ...
            <span className="flex items-center gap-1.5">
              <BookOpen className="w-4 h-4" />PropCoupons Editorial
            </span>
          </div>
        </div>
      </div>
          ...
          <article className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-gray-100 p-7 shadow-sm">
              {paragraphs.map((para, idx) => (
                <p key={idx} className="text-gray-600 leading-relaxed mb-4 text-sm whitespace-pre-wrap">
                  {para}
                </p>
              ))}
            </div>
          ...
          <aside className="space-y-5">
            ...
            <div className="bg-gradient-to-br from-[#060d1f] to-[#0d1f3c] rounded-2xl p-5 text-white border border-[#1a3a5c]">
              ...
              <p className="text-slate-400 text-xs mb-4">
                5,000+ traders get new codes every Tuesday. Free.
              </p>
              {subStatus === 'done' ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
              ...
              <Link
                href="/"
                className="block text-center text-xs font-black text-blue-600 hover:underline mt-3"
              >
                View All 20+ Deals →
              </Link>
            </div>
